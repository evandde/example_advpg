...
G4VPhysicalVolume *DetectorConstruction::Construct()
{
    ...
    // Source
    auto sourceSize = 1. * cm;
    auto solSource = new G4Tubs("Source", 0., .5 * sourceSize, 5 * sourceSize, 0. * deg, 360. * deg);
    auto lvSource = new G4LogicalVolume(solSource, matBGO, "Source");
    lvSource->SetVisAttributes(visCyanSol);
    new G4PVPlacement(G4Translate3D(30. * cm, -5. * cm, -10. * cm) * G4RotateX3D(45. * deg), lvSource, "Source", lvWorld, false, 0);
    ...
    new G4PVPlacement(0, G4ThreeVector(0., 0., .15 * detectorSize), lvDetector, "Detector", lvWorld, false, 0);
